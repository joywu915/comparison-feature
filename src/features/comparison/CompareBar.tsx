...
export function CompareBar({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-[#e9e9e9] shadow-[0_-4px_16px_rgba(0,0,0,0.10)]">
      <div className="mx-auto max-w-[1170px] px-6 py-3 flex items-center gap-4">
        ...
        <div className="flex items-center gap-3 flex-1 overflow-x-auto scrollbar-none">
          ...
          {Array.from({ length: maxProducts - products.length }).map((_, i) => (
            <div
              key={i}
              className="flex-shrink-0 w-[140px] h-[52px] border-2 border-dashed border-[#cfcfcf] rounded-sm"
            />
          ))}
        </div>
